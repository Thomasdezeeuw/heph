//! Unix datagram socket related types.
//!
//! See [`UnixDatagram`].

#![allow(unused_imports, unused_variables)] // FIXME: remove.
...
//use crate::net::convert_address; // TODO: needd?
use crate::{actor, rt};

/// The unconnected mode of an [`UnixDatagram`].
#[allow(missing_debug_implementations)]
#[allow(clippy::empty_enum)]
pub enum Unconnected {}

/// The connected mode of an [`UnixDatagram`].
#[allow(missing_debug_implementations)]
#[allow(clippy::empty_enum)]
pub enum Connected {}
...
///
/// To create a UDP socket [`UnixDatagram::bind`] can be used, this will bind the
...
///     let mut socket = UnixDatagram::bind(&mut ctx, local)?;
...
///     let mut socket = UnixDatagram::bind(&mut ctx, local_address)
...
pub struct UnixDatagram<M = Unconnected> {
    /// Underlying Unix datagram socket, backed by Mio.
...
impl UnixDatagram {
    /// Create a Unix datagram socket binding to the `path` address.
    ///
    /// # Notes
    ///
    /// The UDS socket is also [bound] to the actor that owns the
    ...
    where
        ...
        P: AsRef<Path>,
    {
        let socket = net::UnixDatagram::bind(path.as_ref())?;
        UnixDatagram::setup_socket(ctx.runtime(), socket)
    }

    pub fn unbound<M, RT, P>(
        ctx: &mut actor::Context<M, RT>,
    ) -> io::Result<UnixDatagram<Unconnected>>
    where
        RT: rt::Access,
    ...
    /* TODO: return `UnboundUnixDatagram` here?
    pub fn pair<M, RT, P>(
        ctx: &mut actor::Context<M, RT>,
    ) -> io::Result<(UnixDatagram<Unconnected>, UnixDatagram<Unconnected>)>
    where
        RT: rt::Access,
        P: AsRef<Path>,
    {
        let (socket1, socket2) = net::UnixDatagram::pair()?;
        let socket1 = UnixDatagram::setup_socket(ctx.runtime(), socket1)?;
        let socket2 = UnixDatagram::setup_socket(ctx.runtime(), socket2)?;
        Ok((socket1, socket2))
    }
    */

    fn setup_socket<RT>(rt: &mut RT, mut socket: net::UnixDatagram) -> io::Result<UnixDatagram>
    where
        RT: rt::Access,
    {
        rt.register(&mut socket, Interest::READABLE | Interest::WRITABLE)?;
        #[cfg(target_os = "linux")]
...
impl<M> UnixDatagram<M> {
    /// Connects the UDS socket by setting the default destination and limiting
    /// packets that are read, written and peeked to `path`.
    pub fn connect<P>(self, path: P) -> io::Result<UnixDatagram<Connected>>
    where
        P: AsRef<Path>,
    {
        self.socket.connect(path.as_ref()).map(|()| UnixDatagram {
            socket: self.socket,
            mode: PhantomData,
        })
    }

    /* TODO: add this, blocked on https://github.com/tokio-rs/mio/issues/1527
     * which is blocked on https://github.com/rust-lang/rust/issues/65275.
...
impl UnixDatagram<Unconnected> {
        ...
        B: Bytes,
    {
        debug_assert!(
            ...
            "called `UnixDatagram::try_recv_from` with an empty buffer"
        );
        ...
        B: BytesVectored,
    {
        debug_assert!(
            ...
            "called `UnixDatagram::try_recv_from` with empty buffers"
        );
        ...
        B: Bytes,
    {
        debug_assert!(
            buf.has_spare_capacity(),
            "called `UnixDatagram::try_peek_from` with an empty buffer"
        );
        SockRef::from(&self.socket)
        ...
        B: BytesVectored,
    {
        debug_assert!(
            bufs.has_spare_capacity(),
            "called `UnixDatagram::try_peek_from_vectored` with empty buffers"
        );
...
/// The [`Future`] behind [`UnixDatagram::send_to`].
#[derive(Debug)]
#[must_use = "futures do nothing unless you `.await` or poll them"]
pub struct SendTo<'a, 'b> {
    socket: &'a mut UnixDatagram<Unconnected>,
    buf: &'b [u8],
    target: SocketAddr,
}
...
/// The [`Future`] behind [`UnixDatagram::send_to_vectored`].
#[derive(Debug)]
#[must_use = "futures do nothing unless you `.await` or poll them"]
...
/// The [`Future`] behind [`UnixDatagram::recv_from`].
#[derive(Debug)]
#[must_use = "futures do nothing unless you `.await` or poll them"]
...
/// The [`Future`] behind [`UnixDatagram::recv_from_vectored`].
#[derive(Debug)]
#[must_use = "futures do nothing unless you `.await` or poll them"]
...
/// The [`Future`] behind [`UnixDatagram::peek_from`].
#[derive(Debug)]
#[must_use = "futures do nothing unless you `.await` or poll them"]
...
/// The [`Future`] behind [`UnixDatagram::peek_from_vectored`].
#[derive(Debug)]
#[must_use = "futures do nothing unless you `.await` or poll them"]
...
impl UnixDatagram<Connected> {
    ...
    */
        ...
        B: Bytes,
    {
        debug_assert!(
            buf.has_spare_capacity(),
            "called `UnixDatagram::try_recv` with an empty buffer"
        );
        SockRef::from(&self.socket)
            .recv(buf.as_bytes())
            .map(|read| {
        ...
        B: BytesVectored,
    {
        debug_assert!(
            bufs.has_spare_capacity(),
            "called `UnixDatagram::try_recv_vectored` with empty buffers"
        );
        let res = SockRef::from(&self.socket)
        ...
        B: Bytes,
    {
        debug_assert!(
            buf.has_spare_capacity(),
            "called `UnixDatagram::try_peek` with an empty buffer"
        );
        SockRef::from(&self.socket)
            .peek(buf.as_bytes())
            .map(|read| {
        ...
        B: BytesVectored,
    {
        debug_assert!(
            bufs.has_spare_capacity(),
            "called `UnixDatagram::try_peek_vectored` with empty buffers"
        );
...
/// The [`Future`] behind [`UnixDatagram::send`].
#[derive(Debug)]
#[must_use = "futures do nothing unless you `.await` or poll them"]
pub struct Send<'a, 'b> {
    socket: &'a mut UnixDatagram<Connected>,
    buf: &'b [u8],
}
...
/// The [`Future`] behind [`UnixDatagram::send_vectored`].
#[derive(Debug)]
#[must_use = "futures do nothing unless you `.await` or poll them"]
...
/// The [`Future`] behind [`UnixDatagram::recv`].
#[derive(Debug)]
#[must_use = "futures do nothing unless you `.await` or poll them"]
pub struct Recv<'a, B> {
    socket: &'a mut UnixDatagram<Connected>,
...
/// The [`Future`] behind [`UnixDatagram::recv_vectored`].
#[derive(Debug)]
#[must_use = "futures do nothing unless you `.await` or poll them"]
...
/// The [`Future`] behind [`UnixDatagram::peek`].
#[derive(Debug)]
#[must_use = "futures do nothing unless you `.await` or poll them"]
pub struct Peek<'a, B> {
    socket: &'a mut UnixDatagram<Connected>,
...
impl<M, RT: rt::Access> actor::Bound<RT> for UnixDatagram<M> {
    type Error = io::Error;
