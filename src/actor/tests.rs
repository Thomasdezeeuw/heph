...
#[test]
fn actor_future() {
    let (actor, actor_ref) = ActorFuture::new(NoSupervisor, actor_fn(ok_actor), ());
...
fn erroneous_actor_process() {
    ...
    let (actor, _) = ActorFuture::new(supervisor, actor_fn(error_actor), true);
    let mut actor = pin!(actor);
...
fn restarting_erroneous_actor_process() {
    ...
    let (actor, actor_ref) = ActorFuture::new(supervisor, actor_fn(error_actor), true);
    let mut actor = pin!(actor);
...
fn panicking_actor_process() {
    ...
    let (actor, _) = ActorFuture::new(supervisor, actor_fn(panic_actor), true);
    let mut actor = pin!(actor);
...
fn restarting_panicking_actor_process() {
    ...
    let (actor, actor_ref) = ActorFuture::new(supervisor, actor_fn(panic_actor), true);
    let mut actor = pin!(actor);
