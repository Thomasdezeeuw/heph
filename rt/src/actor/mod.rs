...
fn format_name(full_name: &'static str) -> &'static str {
    ...
    match (name.find(GEN_FUTURE), name.find(GENERIC_START)) {
        (Some(start_index), Some(i)) if start_index < i => {
            ...
            // `deadline_actor<heph_rt::actor::context::ThreadLocal>` to
            // `deadline_actor`.
            if name.ends_with('>') {
        ...
        _ => {
            ...
            // `heph_rt::net::tcp::server::TcpServer<2_my_ip::conn_supervisor, fn(heph::actor::context_priv::Context<!>, heph::net::tcp::stream::TcpStream, std::net::addr::SocketAddr) -> core::future::from_generator::GenFuture<2_my_ip::conn_actor::{{closure}}>, heph::actor::context::ThreadLocal>`
            // to `heph_rt::net::tcp::server::TcpServer`.
            if let Some(start_index) = name.find(GENERIC_START) {
                name = &name[..start_index];

                if let Some(start_index) = name.rfind("::") {
                    // Next we remove the module path, e.g. from
                    // `heph_rt::net::tcp::server::TcpServer` to `TcpServer`.
