...
//! It returns an iterator that iterates over the metrics in (metric name,
//! metric value) pairs.
//!
...
/// Access to metrics.
pub trait Metrics {
    /// Container type for the metrics that are collected. The contains must be
    /// an iterator that iterates over the (metric name, metric value) pairs.
...
macro_rules! create_metric {
    (
        ...
        impl$( < $( $generic )+ > )? crate::metrics::Metrics for $for_ty$( < $( $generic )+ > )? {
            type Metrics = impl Iterator<Item = (&'static str, crate::metrics::Metric)>;
