...
use heph_http::{self as http, HttpServer, Method, Request, Response, Router};
...
async fn http_actor(
    ...
    // FIXME: pass as argument to actor.
    let router = Router::new(not_found)
        .add_route(Method::Get, "/", index)
        .add_route(Method::Get, "/other_page", index)
        .add_route(Method::Post, "/post", index);
    ...
    loop {
        ...
        let response = match fut.await? {
            ...
            Err(err) => {
                ...
                err.response().with_body(OneshotBody::from("Bad request"))
            }
        };
